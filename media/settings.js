...
(function (root) {
  ...
  function providerLogoMarkup(id) {
    ...
    if (!path) return "";
    return `<svg class="provider-logo" viewBox="0 0 24 24" fill="currentColor" fill-rule="evenodd" aria-hidden="true"><path d="${path}"></path></svg>`;
  }
  ...
  const ROWS = [
    ...
    },
    {
      id: "aboutHostProduct",
      category: "about",
      title: (s) => (s && s.hostKind === "desktop") ? "Grok Build Desktop" : "Grok30m",
      kind: "value",
      visible: (s, env) => remoteAbout(s, env),
