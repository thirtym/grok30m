 ...
 * everything but name.
 *
 * **Fires on a first-ever run, and on an upgrade that never recorded a
 * correction.** There is no way to ask where a view lives — `resetViewLocation`
 ...
 *
 * What CAN be established is that we have not placed it: an install with no
 * stored state has never been interacted with, so wherever the view sits, nobody
 * chose it. The same is true of an upgrade from a build that did not write
 * {@link VIEW_PLACEMENT_KEY} (Grok30m 1.5.7 → 2.0.x): session meta already fills
 * `globalState`, so `isFirstEverRun` is false, but the move never ran. Once a
 * correction has been recorded, every later run leaves the layout alone —
 * including for someone who moved the chat with the editor's own Move To.
 */
export function viewPlacementCorrection(opts: {
  ...
  /** True once a previous launch actually issued the correction. */
  alreadyCorrected?: boolean;
}): { containerId: string; panelPosition: PanelPosition | null } | null {
  // A 1.5.7 window that upgraded in place is not a first-ever run (session
  // meta, install id, …) but never recorded a placement attempt — Cursor then
  // leaves chat in Explorer / the right-hand bar it refuses. Run the move
  // once; after that, the record is the stop.
  if (!opts.isFirstEverRun && opts.alreadyCorrected) return null;
