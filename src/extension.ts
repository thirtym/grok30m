...
async function ensureViewPlacement(
  ...
  try {
    ...
    const alreadyCorrected = Boolean(
      context.globalState.get<PlacementRecord>(VIEW_PLACEMENT_KEY)?.correctedByVersion,
    );
    ...
    if (!target) {
      ...
      log(
        `no move — version=${version}, firstEverRun=${isFirstEverRun}, alreadyCorrected=${alreadyCorrected}`,
      );
      return;
    }
