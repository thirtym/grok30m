...
describe("the one automatic placement correction", () => {
  ...
  it("does not fire again after a correction has been recorded", () => {
    expect(
      viewPlacementCorrection({
  ...
  it("does fire on an upgrade that never recorded a correction", () => {
    // 1.5.7 → 2.0.x: globalState already has session meta / install id, so this
    // is not a first-ever run, but VIEW_PLACEMENT_KEY was never written.
    expect(
      viewPlacementCorrection({
        availableCommands: CURSOR,
        isFirstEverRun: false,
