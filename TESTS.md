...
`test/provider-enumerations.test.ts` checks all four identities across host, desktop validation and renderer capabilities while keeping `ACP_PROVIDERS` at its legacy three ids. `test/muse-backend.test.ts`, `test/muse-session.test.ts` and `test/muse-lifecycle.test.ts` cover the SDK-backed adapter, listing/resume, Windows launchers, teardown and effort changes; `test/muse-effort-request.test.ts` checks the host request path. `test/muse-provider.dom.test.ts` gates Muse on advertisement and hides unsupported actions. `test/muse-install.test.ts` and `test/muse-install.dom.test.ts` cover the desk-only installer message, capability gate, host confirmation, fixed commands, persistent missing state on sidebar/desktop/remote, and Re-check without consent. `test/muse-clear-history.test.ts` keeps its undeletable history; `test/muse-approvals.test.ts` and `test/command-session-grants.test.ts` preserve Muse-owned choices instead of adding host program grants. `test/device-login.test.ts`, `test/device-login-cloud-fixes.test.ts` and `test/muse-cli-version.test.ts` pin sign-in on desk/phone and installed-CLI version reporting. `test/cli-path-discovery.test.ts` and `test/cli-process.test.ts` cover Windows discovery and quoted shim invocation; the Muse session tests cover its separate SDK launch path. `test/provider-consent-host.test.ts` keeps an unconnected provider's binary unspawned and stores an empty `grok.providerConnections.v2` instead of importing the old flags. `test/provider-needs-login.test.ts` drives the model warm-up and history listing into account state. `test/muse-browser-login.test.ts` opens the desk browser for Muse only. `test/auth-renewal-resumes.test.ts` asserts that Connect marks the credential unproven before the first frame that says connected, once per terminal provider.
